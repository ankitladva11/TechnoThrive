...
if uploaded_file is None:
    data = pd.read_csv("D:/Programming/Streamlit/TechnoThrive/features/customer_segementation/data/data.csv", encoding='latin-1')
    ...
    st.image("D:/Programming/Streamlit/TechnoThrive/features/customer_segementation/data/image.png", width=750)
...
# Create sidebar to select features
    ...
    transaction_id = st.sidebar.selectbox("Select Transaction ID:", df.columns,index=0)

    st.sidebar.subheader("👥 Customer ID")
    customer_id = st.sidebar.selectbox("Select Customer ID:", df.columns,index=6)

    st.sidebar.subheader("💰 Revenue Variable")
    revenue = st.sidebar.selectbox("Select Revenue Variable", df.columns,index=5)

    st.sidebar.subheader("🌍 Region Variable")
    region = st.sidebar.selectbox("Select Region Variable", df.columns,index=7)
...
def market_basket():
    ...
    item = st.sidebar.selectbox("Select Item Description:", df.columns,index=2)
    st.sidebar.subheader("👤 Customer ID")
    customer_id = st.sidebar.selectbox("Select Customer ID:", df.columns,index=6)
    st.sidebar.subheader("🛍 Quantity")
    quantity = st.sidebar.selectbox("Select Quantity:", df.columns,index=3)
    st.sidebar.subheader("💰 Revenue Variable")
    revenue = st.sidebar.selectbox("Select Revenue Variable", df.columns,index=5)
    c2 = st.empty()
    ...
    # st.write(first_choice )
    st.dataframe(first_choice)
    fig = px.bar(first_choice, x='count', y=item, title="Top Ten First Choice")
...
side = st.sidebar.selectbox("Select your page", ["Home", "Data Exploration", "RFM Analysis", "Market Basket Analysis"])
if side == "Home":
    home()
